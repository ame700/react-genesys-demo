...
export default function useSdk() {
  ...
  async function initWebrtcSDK(authData: IAuthData) {
    const options: ISdkConfig = {
      ...
      logLevel: 'info',
      wsHost: 'wss://streaming.mec1.pure.cloud',
    };
  ...
  function connectEventHandlers() {
    webrtcSdk.on('ready', () => {
      startSoftphoneSession("+97144501235" , webrtcSdk)
    } );
  ...
  function startSoftphoneSession(phoneNumber: string , sdkIns? : any) {
    if (!phoneNumber) {
      console.error('Must enter a valid phone number.');
      return;
    }
    setTimeout(() => {
      let sdkInstance = ((sdk != null) ? sdk : sdkIns);
      sdkInstance.startSoftphoneSession({ phoneNumber });
    }, 500);
  }
